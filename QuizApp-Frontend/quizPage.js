...
async function fetchQuestions(quizType) {
    ...
    try {
        const response = await fetch(`http://localhost:8080/api/quizType/${quizType}`); // API URL
        if (!response.ok) {
...
window.onload = function () {
    const quizType = localStorage.getItem("quizType"); // Get saved quiz type    
    const quizHead = document.getElementById('h1_heading');
    quizHead.textContent = quizType.toUpperCase() + " QUIZ"+ " 🎯";


    if (quizType) {
        fetchQuestions(quizType);
    } else {
        alert("No quiz type selected!");
    }
};
